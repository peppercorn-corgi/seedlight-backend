 ...
 *   npx tsx scripts/pregenerate-exegesis.ts --start-hour 0 --end-hour 6        # only run 00:00-06:00
 *   npx tsx scripts/pregenerate-exegesis.ts --start-hour 22 --end-hour 6       # cross-midnight: 22:00-06:00
 */
...
function isWithinTimeWindow(): boolean {
  ...
  // Cross-midnight: e.g. start=22, end=6 → valid when hour>=22 OR hour<6
  return hour >= START_HOUR! || hour < END_HOUR!;
}
...
释经原则——基于圣经综合解读（综合解经），这是华人教会最广泛接受的释经方法：
...
- **信心产生行为**：真实的信仰必然带来生命更新和行为改变，不是单纯守约或遵守规条

共同要求：写成一段话，适合手机碎片时间阅读。语气像关怀的牧者在安静地与人谈心，不要写成论文。在每段文字中，用 **加粗标记** 包裹1-2句最触动心灵的话（如"**这句话会被高亮**"）。高亮的必须是直接给人安慰、温暖或力量的句子——那种让人想停下来反复品味的话，而不是分析性、比较性或知识性的观点。

四个版本的区别（经文背景的篇幅随信仰程度递减）：
- **seeker** (慕道友, 150-250字): 用2-3句话介绍经文背景（谁写的、写给谁、当时处境），帮助完全不了解圣经的读者建立上下文。通俗易懂，避免教会术语，从生活经验出发解释经文含义。如果是旧约经文，自然地提到新约中类似的教导或耶稣的话作为呼应。
- **new_believer** (初信者, 150-220字): 用1-2句话简述经文背景，鼓励引导，逐步引入信仰概念并简明解释，帮助建立信仰根基。建立旧约到新约的连接，帮助初信者看到整本圣经是一个完整的故事。
- **growing** (成长中, 120-200字): 用1句话点明背景即可（读者已有基本圣经知识），将篇幅留给神学背景、原文含义（附通俗解释），鼓励灵修习惯。明确展示旧约如何指向新约、新约如何诠释旧约，帮助建立整全的圣经观。
- **mature** (成熟信徒, 100-180字): 无需介绍背景（读者熟悉圣经），直接进入深层属灵洞见，可用神学术语，引用原文帮助理解。深入展示新旧约的互释关系和以基督为中心的解经脉络。
...
async function main() {
  ...
  log(`=== pregenerate-exegesis ${stoppedByTimeWindow ? "stopped (time window)" : "done"} (${totalMin} min) ===`);
